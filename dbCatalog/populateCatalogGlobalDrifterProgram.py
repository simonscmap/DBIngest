...
"""GlobalDrifterProgram data Catalog Table"""
server = 'Rainier'
tableName = 'tblGlobalDrifterProgram'
rawFilePath = cfgv.rep_global_drifter_program_raw
rawFileName = 'global_drifter_program_metadata.xlsx'
...
""" Strings """
DB='Opedia'
Dataset_Name = 'tblGlobalDrifterProgram'
Dataset_Long_Name = dataset_metadata.iloc[0]['dataset_long_name']
Data_Source = dataset_metadata.iloc[0]['dataset_source']
Distributor = 'National Oceanic and Atmospheric Administration Atlantic Oceanography and Meteorological Laboratory'
...
temporal_res_list = list('2') * len(vars_metadata) # Irregular
...
Study_Domain_ID_list = ['1'] * len(vars_metadata) # physics
...
# print(len(DB), len(Dataset_Name), len(Dataset_Long_Name), len(Variables), len(Data_Source), len(Distributor), len(Description), len(Climatology),len(server))
...
# ssf.buildVarDFLargeTables(tableName,server)
""" add to Datasets_Cruises table """
# cF.lineInsert('tblDataset_Cruises', '(Dataset_ID, Cruise_ID)', '(,)',server)
