...
rawFilePath = cfgv.rep_MODIS_PAR_daily_raw
...
def makeMODIS_PAR_daily(rawFilePath, rawFileName, tableName):
    ...
    df['time'] = pd.to_datetime(xdf.attrs['time_coverage_start']).date().strftime('%Y-%m-%d 00:00:00')
...
# export_path = makeMODIS_PAR_daily(rawFilePath, os.path.basename(rawFileName), tableName)
...
# df,xdf = makeMODIS_PAR_daily(rawFilePath, os.path.basename(rawFileName), tableName)
